...
class ANETcaptions(object):
    ...
    def __init__(self, ground_truth_filenames=None, prediction_filename=None,
        ...
        self.only_proposals = only_proposals    # True
        self.verbose = verbose                  # True
        self.tious = tious                      # [0.3, 0.5, 0.7, 0.9]
        self.max_proposals = max_proposals      # 100
        self.pred_fields = prediction_fields    # ['results', 'version', 'external_data']
        self.ground_truths = self.import_ground_truths(ground_truth_filenames)   # [val_1_dict, val_2_dict]
        self.prediction = self.import_prediction(prediction_filename)            # dict
    ...
    def import_ground_truths(self, filenames):
        ...
        for filename in filenames:
            ...
            self.n_ref_vids.update(gt.keys())   # update set
            gts.append(gt)
        if self.verbose:
    ...
    def check_gt_exists(self, vid_id):
        for gt in self.ground_truths:    # [val_1_dict, val_2_dict]
            if vid_id in gt:
                return True
        return False
    ...
    def get_gt_vid_ids(self):
        ...
        for gt in self.ground_truths:
            vid_ids |= set(gt.keys())  # set={},filter repeat vid_ids
        return list(vid_ids)           # []
    ...
    # 主要计算的是候选片段caption与gt片段caption，即是否所有的gt片段都被预测到
    def evaluate_detection(self, tiou):
        ...
        # 每个视频均给予一个精确率和召回率
        recall = [0] * len(gt_vid_ids)       # 查全率
        precision = [0] * len(gt_vid_ids)    # 查准率
        for vid_i, vid_id in enumerate(gt_vid_ids):
            ...
            for gt in self.ground_truths:
                if vid_id not in gt:
                    continue
                refs = gt[vid_id]
                ref_set_covered = set([])     # not repeat set
        ...
        for scorer, method in self.scorers:
            ...
            for vid_id in gt_vid_ids:
                ...
                if len(res[vid_id]) == 0 or len(gts[vid_id]) == 0:
                    ...
                    # 计算一个视频的caption得分,score是对于一个视频来说是类似平均得分, scores是对于每一个片段的得分
...
if __name__=='__main__':
    ...
    # parser.add_argument('-s', '--submission', type=str,  default='../log/train_cap/0815142747_audio_video_text/captioning_results_val_1_e17.json',
    #                     help='sample submission file for ActivityNet Captions Challenge.')
    parser.add_argument('-s', '--submission', type=str,  default='../results/captioning_results_val_1_ideal_100.json',
                        help='sample submission file for ActivityNet Captions Challenge.')
    parser.add_argument('-r', '--references', type=str, nargs='+', default=['../data/val_1_no_missings_subs.json'],
    ...
    parser.add_argument('-ppv', '--max-proposals-per-video', type=int, default=100,
