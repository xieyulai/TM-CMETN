...
def clean_text(text):
    ...
    text = text.replace('​​', '')
    text = text.replace('(Music)', '')
    text = text.replace('(Performs criss cross)', '')
    text = text.replace('[MUSIC PLAYING]', '')
    text = text.replace('(fun music)', '')
    text = text.replace('- ', '')
    text = text.replace('(laughing)', '')
    text = text.replace('[編輯本週視頻的時候,', '')
    text = text.replace('(jazz music)', '')
    text = text.replace('[Музика]', '')
...
def parse_sub(in_stream):
    ...
    text = text +in_stream.readline().replace('\n', '')
...
if __name__ == "__main__":
    ...
    subs_folders = [f'../data/subs/asr_translated/']
    save_path = f'../data/asr_translated_1.csv'
